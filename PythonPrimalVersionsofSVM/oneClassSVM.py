...
# Note just one negative instance now!

N2 = 1 # number of negative instances
D = 2 # feature dimension
eps = 1e-8 # select support vectors
random.seed(1) # For reproducibility
data1 = np.random.rand(N1,2)+0.5 # Points
data2 = np.array([[0, 0]]) # exactly one negative point

## generate toy testing data
## This is essentially recall problem now meaning does instance belong to 
## class one or not
...
plt.savefig('oneClassSVM_train_decision.png')
...
plt.savefig('oneClassSVM_test_decision.png')
plt.show()
